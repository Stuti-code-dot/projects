...
train_y = train_y.replace(2,0) # benign
train_y = train_y.replace(4,1) # malignant
train_y
...
train_x = train_x.drop(columns = ['Class','ID'])


# In[ ]:


train_x_np = (train_x.values).astype(int)
train_x_np
...
np.unique(train_x_np,return_counts = True)
...
train_x_rs,train_y_rs = sm.fit_resample(train_x_np,train_y)
...
def genetic(data,labels,i):
    if i==1:
        ...
        estimator = MLPClassifier(hidden_layer_sizes = (1),activation = 'identity',validation_fraction = 0.1,solver = 'lbfgs',n_iter_no_change = 2,tol = 1e-2,batch_size = 4,learning_rate='adaptive',random_state=1,early_stopping = True)
...
train_x_rs = np.delete(train_x_rs,final_list,axis = 1)
